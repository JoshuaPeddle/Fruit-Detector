...
def load_data():
    ...
    # Load data
    train_data = train_loader.flow_from_directory(train_dir, target_size=(64, 64), batch_size=int(sum(n_objects_per_class_train)/1), class_mode='categorical')
    test_data  = test_loader.flow_from_directory(test_dir, target_size=(64, 64), batch_size=int(sum(n_objects_per_class_test)/1), class_mode='categorical')
