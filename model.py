...
def get_model(hp, data_augmentation, img_height, img_width, class_names):
    ...
    else:  model.add(layers.Conv2D(128, (3, 3), activation='relu', input_shape=(img_height, img_width, 3)))

    if hp: model.add(layers.MaxPooling2D(pool_size=hp.Choice('polling1', [ 2,4])))
    else: model.add(layers.MaxPooling2D((2, 2)))

    if hp: model.add(layers.Conv2D(hp.Choice('conv2', [256,512]), (3, 3), activation=hp.Choice('act2', ['relu']), input_shape=(img_height, img_width, 3)))
    ...
    if hp: model.add(layers.Dense(hp.Choice('dense', [128,256]), activation='relu'))
    ...
    if hp: model.add(layers.Dense(len(class_names), activation=hp.Choice('finalact', ['relu', 'softmax'])))
