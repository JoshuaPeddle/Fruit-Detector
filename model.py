...
def get_model(hp, data_augmentation, img_height, img_width, class_names):
    ...
    model.add(BatchNormalization())
    model.add(layers.MaxPooling2D((2, 2)))
    if hp: model.add(layers.Dropout(hp.Choice('sparce_dropout', [0.1, 0.25, 0.4])))
    ...
    if hp: model.add(layers.Dense(hp.Choice('dense', [2048]), activation='relu'))
    else: model.add(layers.Dense(2048, activation='relu'))
    if hp: model.add(layers.Dense(hp.Choice('dense', [2048]), activation='relu'))
    else: model.add(layers.Dense(2048, activation='relu'))
    
    model.add(BatchNormalization())
    if hp: model.add(layers.Dropout(hp.Choice('dense_dropout', [0.1, 0.25, 0.4])))
    else: model.add(layers.Dropout(0.25))

    if hp: model.add(layers.Dense(len(class_names), activation=hp.Choice('finalact', ['softmax'])))
    else: model.add(layers.Dense(len(class_names), activation='softmax'))
    

    if hp:
        model.compile(keras.optimizers.Adam(learning_rate=hp.Choice('learning_rate', [0.0005,0.0001,0.00005,0.00001]), beta_1=0.9, beta_2=0.999, amsgrad=True),
