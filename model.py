...
def get_model(hp, data_augmentation, img_height, img_width, class_names):
    ...
    if hp: model.add(layers.Conv2D(hp.Choice('conv2', [256]), (3, 3), activation=hp.Choice('act2', ['relu']), input_shape=(img_height, img_width, 3)))
    ...
    if hp: model.add(layers.Conv2D(hp.Choice('conv3', [512]), (3, 3), activation='relu'))
    ...
    if hp: model.add(layers.Dense(len(class_names), activation=hp.Choice('finalact', ['relu', 'softmax', 'tanh'])))
