...
def get_model(hp, data_augmentation, img_height, img_width, class_names):
    ...
    else: model.add(layers.Conv2D(128, (3, 3), activation='relu'))

    model.add(layers.MaxPooling2D((2, 2)))

    if hp: model.add(layers.Dropout(hp.Choice('sparce_dropout', [0.1, 0.15])))
    ...
    if hp: model.add(layers.Dense(hp.Choice('dense', [64, 128, 256]), activation='relu'))
    else: model.add(layers.Dense(128, activation='relu'))
    model.add(BatchNormalization())
    if hp: model.add(layers.Dropout(hp.Choice('dense_dropout', [0.1, 0.17])))
